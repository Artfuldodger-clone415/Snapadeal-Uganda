...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const login = async (emailOrPhone: string, password: string) => {
    try {
      ...
      if (error.response?.data?.error?.includes("Merchant account required")) {
        throw new Error("Access denied. This portal is for merchants only.")
      }
  ...
  const register = async (userData: RegisterData): Promise<{ email: string; status: string }> => {
    try {
      const merchantData = { ...userData, role: "merchant" }
      const response = await api.post("/auth/register", merchantData)
